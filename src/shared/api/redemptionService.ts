...
export const redemptionService = {
  getRedemptionsByUserId: async (
    ...
    const { data } = await axiosClient.get("/redemptions", {
      params: { user_id: id },
    });
    return data;
  },

  getRedemptionsById: async (
    id: string
  ): Promise<ApiResponse<Redemption[]>> => {
    const { data } = await axiosClient.get("/redemptions", {
      params: { id: id },
  ...
  updateRedemptionStatus: async (updateData: {
    id: string;
    is_delivered: string;
  }): Promise<ApiResponse<Redemption>> => {
    const { id, ...body } = updateData;
    const { data } = await axiosClient.patch(
      "/redemptions/:id".replace(":id", id),
      body
    );
    return data;
  },
