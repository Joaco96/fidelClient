...
const UserDetail = () => {
  ...
  const {
    ...
    handleApiResponse: deleteUserHandleResponse,
    ...
    if (!rolesSelectRef.current?.value) {
      ...
      if (user_id && rolesSelectRef) {
        const data = await updateUserRole({
          ...
          body: {
            role_id: Number(
              rolesSelectRef.current!.value
            ) as unknown as User["role_id"],
          },
        });
            ...
            <div className="flex items-start justify-between gap-6">
              <div className="w-1/2 flex flex-col bg-[#FFE9D1] p-5 rounded-lg">
                ...
                <button
                  ...
                  className="font-medium py-2 cursor-pointer bg-[#515838] hover:bg-[#484e32] disabled:opacity-50 w-fill text-center h-fit rounded-lg flex justify-center text-white mt-2"
                >
            ...
            <div className="flex gap-6 w-full">
              <div className="w-1/2 justify-center items-center">
                <button
                  ...
                  className=" text-[#f00] py-2 cursor-pointer disabled:opacity-50 text-center hover:bg-[#e9e9e9] font-medium rounded-lg w-full"
                >
                  {deleteUserPending ? "Eliminando..." : "Eliminar cuenta"}
                </button>
              </div>
              <div className="w-1/2"></div>
            </div>
